...
export default {
  ...
  methods: {
    removeItem(idx) {
        //dapat item dari local storage
      let keranjangUserStorage = JSON.parse(
        localStorage.getItem("keranjangUser")
      );

    //map / transformasi data itemnya menjadi id nya aja
      let itemKeranjangUser = keranjangUserStorage.map((item) => item.id);
        // cari index berdasarkan id nya
     let index = itemKeranjangUser.findIndex((id) => id == idx);

    //  potong berdasarkan index yang di dapatkan
